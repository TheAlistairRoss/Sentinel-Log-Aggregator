...
def run_security_scan():
    ...
    # Run bandit
    result = subprocess.run(["bandit", "-r", "sentinel_log_aggregator/", "-q"], 
                          capture_output=True, text=True)
    if result.returncode != 0:
...
def create_git_tag(version: str):
    ...
    # Create tag
    result = subprocess.run(["git", "tag", "-a", tag_name, "-m", f"Release {version}"], 
                          capture_output=True, text=True)
...
def main():
    ...
    parser.add_argument("action", choices=["version", "bump", "release", "build"], 
                       help="Action to perform")
    parser.add_argument("--type", choices=["major", "minor", "patch", "alpha", "beta", "rc"],
