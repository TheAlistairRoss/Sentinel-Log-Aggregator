...
class WorkspaceConfigModel(BaseModel):
    ...
    queries_list: List[str] = Field(
        ...
        description="List of queries this workspace should execute"
    )
...
class QueryDefinitionModel(BaseModel):
    ...
    name: str = Field(
        ...
        description="Query name (lowercase, underscores allowed)"
    )
    ...
    parameters: Dict[str, QueryParameterModel] = Field(
        ...
        description="Query parameters definition"
    )
    query: str = Field(
        ...,
        min_length=10,
    ...
    tags: List[str] = Field(
        default_factory=list,
        description="Query tags for categorization"
    )
    ...
    def validate_tags(cls, v):
        ...
        for tag in v:
            if not tag_pattern.match(tag):
                raise ValueError(f"Invalid tag format: {tag}. Tags must be lowercase, alphanumeric with underscores/hyphens")
        return v
...
class ClientOptionsModel(BaseModel):
    ...
    batch_hours: int = Field(
        ...
        ge=1,
        le=168,
        description="Batch processing time window in hours"
    )
    ...
    max_upload_retries: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum upload retry attempts"
    )
    ...
    enable_telemetry: bool = Field(
        default=True,
        description="Enable Azure SDK telemetry"
    )
...
class WorkspaceCollectionModel(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Optional metadata for the workspace collection"
    )
