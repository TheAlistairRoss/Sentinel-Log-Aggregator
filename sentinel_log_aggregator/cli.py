...
def create_client_options_from_args(args: Any) -> SentinelAggregatorClientOptions:
    ...
    days_back = args.days_back or int(os.getenv("DAYS_AGO", "30"))
    batch_hours = args.batch_hours or int(os.getenv("BATCH_HOURS", "24"))
    max_concurrent = args.max_concurrent_queries or int(os.getenv("MAX_CONCURRENT_QUERIES", "5"))
    ...
    try:
        ...
        async with SentinelAggregatorClient(
            ...
            if workspaces:
                ...
                logger.info(f"🧪 Testing query connectivity to workspace {test_workspace.workspace_name}...")
                
                # Simple test query
                test_query = "print 'Health check test query successful'"
                
                query_result = await client.query_workspace(
                    workspace_id=test_workspace.customer_id,
                    query=test_query
                )
                
                if query_result.succeeded:
                    logger.info(f"✅ Test query successful: {query_result.record_count} records in {query_result.execution_time:.2f}s")
                else:
...
Examples:
  ...
  sentinel-aggregator run --workspace-config workspaces.yaml \\
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"Microsoft Sentinel Log Aggregator {__version__}"
    )
    ...
    parser.add_argument(
        "--dcr-endpoint",
        help="Azure Monitor Data Collection Rule logs ingestion endpoint"
    )
    ...
    run_parser.add_argument(
        "--batch-hours",
        type=int, 
        help="Number of hours per batch (default: 24)"
    )
...
async def main() -> int:
    ...
    try:
        ...
        if args.command == "health":
            ...
            batch_hours = args.batch_hours if args.batch_hours is not None else client_options.batch_hours
            ...
            if workspaces:
                ...
                for i, workspace in enumerate(workspaces, 1):
                    masked_id = workspace.customer_id[:8] + "***" if len(workspace.customer_id) > 8 else "***"
