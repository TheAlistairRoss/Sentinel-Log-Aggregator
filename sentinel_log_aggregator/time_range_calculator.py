...
def _calculate_from_explicit_times(client_options) -> Tuple[datetime, datetime]:
    ...
    if not start_time:
        raise TimeRangeCalculationError(
            "start_time is required when using explicit time range"
        )
...
async def _calculate_from_last_successful(
    ...
    if not all_query_names:
        raise TimeRangeCalculationError(
            "No queries found in workspace configurations"
        )
    ...
    for workspace in workspaces:
        ...
        for query_config in workspace.queries_list:
            ...
            try:
                ...
                logger.error(f"Failed to query last successful run for {query_name} in workspace {workspace_id[:8]}: {e}")
                missing_combinations.append(f"{query_name} (workspace: {workspace_id[:8]}) - error: {e}")
...
async def _query_last_successful_for_query_workspace(
    ...
    try:
        ...
        logger.debug(f"Failed to query last successful run for {query_name}/{workspace_id[:8]}: {e}")
        return None
    ...
    try:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            for i, (batch_start, batch_end) in enumerate(batches, 1):
                ...
                logger.debug(f"  Batch {i}: {batch_start.isoformat()} to {batch_end.isoformat()} ({duration})")
...
def validate_time_configuration(client_options) -> List[str]:
    ...
    try:
        ...
        has_lookback = bool(client_options.lookback_period and client_options.lookback_period != "P30D")  # Default value
        ...
        if time_methods > 1:
            ...
            errors.append(f"Conflicting time specifications: {', '.join(active_methods)}. Use only one method.")
