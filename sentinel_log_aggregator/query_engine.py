...
class SentinelQueryEngine:
    ...
    def __init__(self, client_options: SentinelAggregatorClientOptions, azure_client: SentinelAggregatorClient):
        """
    ...
    def calculate_time_batches(self, days_back: int, batch_hours: int) -> List[Tuple[datetime, datetime]]:
        """
    ...
    async def execute_single_query_with_upload(
        ...
        try:
            ...
            if query_result.succeeded:
                ...
                self.logger.query_end(query_name, workspace_alias, execution.record_count, query_result.execution_time, success=True)
                ...
                if query_result.data:
                    ...
                    transformed_data = self._transform_data_for_upload(query_result.data, workspace_id)
                    ...
                    if upload_result.succeeded:
                        ...
                        self.logger.upload_end(query_name, workspace_alias, upload_result.record_count, upload_result.upload_time, success=True)
                    else:
                        execution.upload_status = UploadStatus.FAILED.value
                        execution.upload_error_message = upload_result.error_message
                        self.logger.error("UPLOAD", upload_result.error_message, query_name=query_name, workspace_alias=workspace_alias)
                
                else:
                    execution.upload_status = UploadStatus.SKIPPED.value
                    execution.uploaded_count = 0
                    self.logger.info(f"No data to upload - Job: {self.job_correlation_id} | Query: {query_name} | Workspace: {workspace_alias}")
            else:
                # Query failed
                ...
                execution.query_duration_seconds = query_result.execution_time
                execution.upload_status = UploadStatus.SKIPPED.value
                self.logger.query_end(query_name, workspace_alias, 0, query_result.execution_time, success=False)
            ...
            self.logger.error("QUERY_ENGINE", str(e), query_name=query_name, workspace_alias=workspace_alias, error_type=error_type)
    ...
    def _transform_data_for_upload(self, data: List[Dict[str, Any]], workspace_id: str) -> List[Dict[str, Any]]:
        """
    ...
    async def execute_batch_queries_with_streaming_upload(
            ...
            for query_name in queries_list:
                if query_name in AVAILABLE_QUERIES:
                    try:
                        ...
                        self.logger.debug(f"Built query '{query_name}' for workspace {workspace_alias}")
        ...
        for i in range(0, len(all_tasks), batch_size):
            ...
            try:
                ...
                if syntax_errors:
                    self.logger.error("SYNTAX", f"{len(syntax_errors)} syntax error(s) detected - stopping execution")
                    for error_exec in syntax_errors:
                        self.logger.error("SYNTAX_DETAIL", error_exec.query_error_message, query_name=error_exec.query_name)
        ...
        successful_queries = len([e for e in all_executions if e.query_status == QueryStatus.SUCCESS.value])
        failed_queries = len([e for e in all_executions if e.query_status == QueryStatus.FAILED.value])
        successful_uploads = len([e for e in all_executions if e.upload_status == UploadStatus.SUCCESS.value])
        failed_uploads = len([e for e in all_executions if e.upload_status == UploadStatus.FAILED.value])
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        successful_queries = len([e for e in self.execution_log if e.query_status == QueryStatus.SUCCESS.value])
        failed_queries = len([e for e in self.execution_log if e.query_status == QueryStatus.FAILED.value])
        ...
        return {
            ...
            "successful_queries": successful_queries,
            "failed_queries": failed_queries,
            "success_rate": (successful_queries / len(self.execution_log)) * 100 if self.execution_log else 0,
