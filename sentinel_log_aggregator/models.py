...
class KQLQueryDefinition:
    ...
    def __init__(self, name: str, destination_stream: str, description: str, stream_name: str, query: str = ""):
        self.name = name
    ...
    def generate_detailed_summary(self) -> Dict[str, Any]:
        ...
        for workspace, queries in workspace_query_stats.items():
            for query_name, query_executions in queries.items():
                successful_executions = [e for e in query_executions if e.query_status == QueryStatus.SUCCESS.value]
                failed_executions = [e for e in query_executions if e.query_status == QueryStatus.FAILED.value]
                successful_uploads = [e for e in query_executions if e.upload_status == UploadStatus.SUCCESS.value]
                failed_uploads = [e for e in query_executions if e.upload_status == UploadStatus.FAILED.value]
                
                # Calculate execution time statistics
                exec_times = [e.query_duration_seconds for e in successful_executions if e.query_duration_seconds > 0]
                ...
                upload_times = [e.upload_duration_seconds for e in successful_uploads if e.upload_duration_seconds > 0]
                ...
                upload_failed_count = records_downloaded - records_uploaded if records_downloaded > records_uploaded else 0
                ...
                query_detail = {
                    ...
                    'startTimeRange': min(e.start_time for e in query_executions).strftime('%Y-%m-%d %H:%M:%S'),
                    'endTimeRange': max(e.end_time for e in query_executions).strftime('%Y-%m-%d %H:%M:%S'),
                    # Keep legacy fields for backward compatibility
                    'workspace': query_executions[0].workspace_alias or workspace[:8] + "..." if len(workspace) > 8 else workspace,
                    ...
                    'records': {
                        ...
                        'uploaded_failed': records_downloaded - records_uploaded if records_downloaded > records_uploaded else 0
                    },
                    'time_range': {
                        'start': min(e.start_time for e in query_executions).strftime('%Y-%m-%d %H:%M'),
