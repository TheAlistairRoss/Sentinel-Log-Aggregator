...
from .security_utils import (
    validate_workspace_id, validate_azure_resource_id, validate_file_path,
    sanitize_log_output, SecureLogger
)
from .validation import validate_workspace_config, WorkspaceCollectionModel
...
class WorkspaceManager:
    ...
    def __init__(self, workspace_configs: Optional[List[WorkspaceConfig]] = None) -> None:
        """
    ...
    def validate_configuration(self) -> List[str]:
        ...
        for i, workspace in enumerate(self.workspaces):
            ...
            if workspace.customer_id in seen_customer_ids:
                errors.append(f"{workspace_identifier}: Duplicate customer_id '{workspace.customer_id}'")
            ...
            try:
                if not workspace.workspace_name:
                    errors.append(f"{workspace_identifier}: Invalid resource_id format - cannot extract workspace name")
            except Exception:
    ...
    def get_subscription_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for workspace in self.workspaces:
            ...
            if sub_id:
                ...
                subscription_summary[sub_id]['workspace_count'] = subscription_summary[sub_id]['workspace_count'] + 1
    ...
    def from_file(cls, config_file: Union[str, Path]) -> 'WorkspaceManager':
        ...
        if file_extension not in ['.yaml', '.yml']:
            raise ValueError(f"Unsupported configuration file format: {file_extension}. Supported formats: .yaml, .yml")
        ...
        with open(config_file, 'r', encoding='utf-8') as f:
            ...
            if isinstance(config_data, dict) and 'workspaces' in config_data:
                ...
                if 'metadata' in config_data:
                    ...
                    logger.debug(f"📊 Configuration metadata: version={metadata.get('version', 'unknown')}")
            else:
        ...
        try:
            ...
            if not isinstance(workspace_data, list):
                raise ValueError(f"Workspace configuration must be a list of workspace objects, got: {type(workspace_data)}")
