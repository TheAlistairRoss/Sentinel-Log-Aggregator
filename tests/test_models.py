...
# Test Query Definitions for Testing
class MockIncidentSummaryQuery(KQLQueryDefinition):
    def __init__(self):
        super().__init__(
            name="test_incident_summary",
            destination_stream="Custom-Test-IncidentSummary_CL",
            description="Simple test query for incident summary testing",
            stream_name="stream_test_incident_summary"
        )
        self.add_parameter("row_level_security_tag", "string", required=False, default="")
    
    def get_query(self) -> str:
        return """
        // Simple test query - gets basic incident data
        SecurityIncident
...
class MockWorkspaceUsageQuery(KQLQueryDefinition):
    def __init__(self):
        super().__init__(
            name="test_workspace_usage",
            destination_stream="Custom-Test-WorkspaceUsage_CL", 
            description="Simple test query for workspace usage testing",
            stream_name="stream_test_workspace_usage"
        )
        self.add_parameter("row_level_security_tag", "string", required=False, default="")
    
    def get_query(self) -> str:
        return """
        // Simple test query - gets basic usage data
        Usage
...
# Create test query instances
TEST_QUERIES = {
    "test_incident_summary": MockIncidentSummaryQuery(),
    "test_workspace_usage": MockWorkspaceUsageQuery()
}
...
class TestPredefinedQueries:
    ...
    def test_yaml_queries_loaded(self):
        ...
        for query_name, query_instance in AVAILABLE_QUERIES.items():
            assert hasattr(query_instance, 'stream_name'), f"Query {query_name} should have stream_name attribute"
            assert query_instance.stream_name, f"Query {query_name} should have a non-empty stream_name"
"""
...
from sentinel_log_aggregator.models import WorkspaceConfig, QueryExecution
...
class TestSentinelQueryEngine:
    ...
    def mock_azure_client(self):
        ...
        client.query_workspace = AsyncMock(return_value=MagicMock(
        ...
        client.upload_logs = AsyncMock(return_value=MagicMock(
    ...
    @pytest.mark.asyncio
    async def test_execute_single_query_with_upload_query_failure(self, query_engine, mock_azure_client):
    ...
    @pytest.mark.asyncio
    async def test_batch_execution_summary_generation(self, query_engine, sample_workspaces, mock_azure_client):
        ...
        with patch.object(query_engine, 'execute_batch_queries_with_streaming_upload') as mock_batch:
...
class TestQueryEnginePerformance:
    ...
    @pytest.mark.slow
    def test_memory_usage_large_datasets(self, query_engine):
        """Test memory usage with large datasets."""
        # Test that the engine doesn't consume excessive memory with large datasets
        # This would involve monitoring memory usage during processing
        
        # Create large mock dataset
        large_dataset = []
        for i in range(10000):
            large_dataset.append({
                "TimeGenerated": f"2023-01-01T{i % 24:02d}:00:00Z",
                "IncidentNumber": i,
                "Data": f"Large data string {i}" * 100  # Simulate large records
            })
        
        # Note: This test would need to be updated to test actual memory usage
        # rather than non-existent add_row_level_security_tag method
...
from sentinel_log_aggregator.client_options import SentinelAggregatorClientOptions
from sentinel_log_aggregator.responses import QueryStatus, UploadStatus
from sentinel_log_aggregator.exceptions import QueryExecutionError, DataIngestionError
...
class TestSentinelAggregatorClient:
    ...
    async def test_query_workspace_success(self, client):
        ...
        mock_response.tables = [
            MagicMock(
                ...
                rows=[
                    ["2023-01-01T00:00:00Z", 1001],
                    ["2023-01-01T00:01:00Z", 1002]
                ]
            )
        ]
        
        # Mock the internal _logs_query_client attribute instead of the property
        mock_logs_client = MagicMock()
        mock_logs_client.query_workspace = AsyncMock(return_value=mock_response)
        mock_logs_client.close = AsyncMock()  # Add async close method
    ...
    async def test_upload_logs_success(self, client):
        ...
        # Mock the internal _logs_ingestion_client attribute instead of the property
        mock_ingestion_client = MagicMock()
        mock_ingestion_client.upload = AsyncMock(return_value=MagicMock())
        mock_ingestion_client.close = AsyncMock()  # Add async close method
    ...
    async def test_upload_logs_empty_data(self, client):
        ...
        result = await client.upload_logs(
            data=[],
            stream_name="Custom-Test_Empty_CL"
        )
...
from sentinel_log_aggregator.validation import (
    WorkspaceConfigModel, QueryDefinitionModel, ClientOptionsModel,
...
from sentinel_log_aggregator.security_utils import (
    validate_kql_query, validate_workspace_id, validate_azure_resource_id,
    validate_file_path, sanitize_log_output, sanitize_user_input,
    generate_correlation_id, hash_sensitive_data, SecurityError, SecureLogger
)
...
class TestQueryDefinitionValidation:
    ...
    def test_invalid_parameter_default_type(self):
        ...
        query_data = {
            ...
            'stream_name': 'stream_invalid_param_query',
            'report_name': 'report_test',
            'query': 'SecurityIncident | take 10',
            'parameters': {
                'count': {
                    'type': 'int',
                    'default': 'not_an_int'  # Wrong type for default
                }
            }
        }
...
class TestWorkspaceSet:
    ...
    def setup_test_queries(self, monkeypatch):
        """Add test queries to AVAILABLE_QUERIES for testing."""
        # Import the module where AVAILABLE_QUERIES is defined
        from sentinel_log_aggregator import models
        
        # Create a copy of existing queries and add test queries
        original_queries = models.AVAILABLE_QUERIES.copy()
        updated_queries = original_queries.copy()
        updated_queries.update(TEST_QUERIES)
        
        # Patch AVAILABLE_QUERIES in the models module
        monkeypatch.setattr(models, 'AVAILABLE_QUERIES', updated_queries)
    ...
    def test_filter_by_alias(self, workspace_set):
        ...
        assert filtered.count() == 1
        # Removed first_id assertion since method was removed
    
    def test_has_report(self, workspace_set):
        """Test filtering by report."""
        incident_workspaces = workspace_set.has_query("test_incident_summary")
        assert incident_workspaces.count() == 2
        assert incident_workspaces.aliases() == ["prod", "dev"]
        
        usage_workspaces = workspace_set.has_query("test_workspace_usage")
    ...
    def test_details(self, workspace_set):
        ...
        assert set(first_detail['queries']) == {"test_incident_summary", "test_workspace_usage"}
    ...
    def test_empty_workspace_set(self):
        ...
        # Removed first_id and first_alias assertions since methods were removed
...
class TestWorkspaceManager:
    ...
    def test_for_report(self, sample_workspaces):
        ...
        incident_workspaces = manager.for_query("test_incident_summary")
        assert incident_workspaces.count() == 1
        assert incident_workspaces.aliases() == ["prod"]
        
        usage_workspaces = manager.for_query("test_workspace_usage")
    ...
    def test_unique_reports(self, sample_workspaces):
        ...
        expected_reports = {"test_incident_summary", "test_workspace_usage"}
...
class TestModelsMissingCoverage(unittest.TestCase):
    """Test class to cover missing lines in models.py for 100% coverage."""
    
    def test_kql_query_definition_to_dict_with_defaults(self):
        """Test to_dict method with default parameters - covers lines 150-160."""
        ...
        class TestQuery(KQLQueryDefinition):
            def __init__(self):
                ...
                self.add_parameter("param_with_default", "string", required=False, default="default_value")
                self.add_parameter("param_no_default", "string", required=False)
        ...
        assert "param_with_default" in result["parameters"]
        assert result["parameters"]["param_with_default"]["default"] == "default_value"
        
        # Check that parameters without defaults don't have default key
        assert "param_no_default" in result["parameters"]
        assert "default" not in result["parameters"]["param_no_default"]
    
    def test_kql_query_definition_missing_required_parameter(self):
        """Test query building with missing required parameter - covers line 142."""
        from sentinel_log_aggregator.models import KQLQueryDefinition
        ...
        class TestQuery(KQLQueryDefinition):
            ...
            def get_query(self) -> str:
                return "SELECT * FROM table WHERE field = {required_param}"
    ...
    def test_initialize_query_registry_exception_handling(self):
        """Test _initialize_query_registry exception handling - covers lines 396-399."""
        from unittest.mock import patch, MagicMock
        
        # Mock the query_registry import inside the function
        with patch('sentinel_log_aggregator.query_registry.query_registry') as mock_registry:
            # Create a mock dict that raises an exception when trying to set items
            failing_dict = MagicMock()
            failing_dict.__setitem__.side_effect = Exception("Failed to register query")
            mock_registry._queries = failing_dict
            
            # Mock AVAILABLE_QUERIES with a test query
            mock_query_def = MagicMock()
            
            with patch('sentinel_log_aggregator.models.AVAILABLE_QUERIES', {'test_query': mock_query_def}):
                # Import and call the function - it should handle the exception gracefully
                from sentinel_log_aggregator.models import _initialize_query_registry
                
                # This should not raise an exception but should log a warning
                _initialize_query_registry()
    
    def test_load_queries_from_yaml_directory_not_exists(self):
        """Test load_queries_from_yaml when directory doesn't exist - covers lines 363-364."""
        from sentinel_log_aggregator.models import load_queries_from_yaml
        from pathlib import Path
        
        # Use a non-existent directory
        non_existent_dir = Path("/non/existent/directory")
        queries = load_queries_from_yaml(non_existent_dir)
        
        # Should return empty dict when directory doesn't exist
        assert queries == {}
    
    def test_load_queries_from_yaml_file_processing_error(self):
        """Test load_queries_from_yaml with file processing error - covers lines 376-377."""
        from sentinel_log_aggregator.models import load_queries_from_yaml
        from pathlib import Path
        from unittest.mock import patch, MagicMock
        import tempfile
        ...
        # Create a temporary directory with a YAML file
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('yaml.safe_load', side_effect=Exception("YAML parsing error")):
                queries = load_queries_from_yaml(temp_path)
                
                # Should return empty dict and log warning
                assert queries == {}

    def test_query_execution_get_error_message(self):
        """Test QueryExecution.get_error_message method - covers lines 211-216."""
        from sentinel_log_aggregator.models import QueryExecution
        from datetime import datetime
        
        # Test with query error message
        ...
        self.assertEqual(execution1.error_message, "Query failed")
        
        # Test with upload error message (no query error)
        ...
        self.assertEqual(execution2.error_message, "Upload failed")
        
        # Test with both messages (query error takes precedence)
        ...
        self.assertEqual(execution3.error_message, "Query failed")
        
        # Test with no error messages
        execution4 = QueryExecution(
            job_correlation_id="test-job",
            execution_id="exec-4",
            workspace_id="test-workspace",
            query_name="test_query",
        ...
        self.assertEqual(execution4.error_message, "")

    def test_kql_query_definition_build_query_edge_cases(self):
        """Test KQLQueryDefinition.build_query edge cases - covers lines 142-146."""
        from sentinel_log_aggregator.models import KQLQueryDefinition
        
        # Create a query with optional parameters with defaults
        query = KQLQueryDefinition(
            name="test_query",
            destination_stream="Custom-Test_CL",
            description="Test query",
            stream_name="Custom-Test_CL"
        )
        query.add_parameter("optional_param", "string", required=False, default="default_value")
        query.add_parameter("empty_optional", "string", required=False, default="")
        
        # Mock the get_query method
        query.get_query = lambda: "SELECT * WHERE param = '{optional_param}' AND other = '{empty_optional}'"
        
        # Test with empty parameters dict - should use defaults
        result = query.build_query({})
        self.assertIn("default_value", result)
        self.assertIn("''", result)  # Empty default becomes empty string
        
        # Test with None values provided - should convert None to string "None"
        result2 = query.build_query({"optional_param": None, "empty_optional": None})
        self.assertIn("None", result2)  # None gets converted to string "None"

    def test_batch_execution_summary_get_detailed_summary(self):
        """Test BatchExecutionSummary.get_detailed_summary method - covers lines 259-344."""
        from sentinel_log_aggregator.models import BatchExecutionSummary, QueryExecution, QueryStatus, UploadStatus
        ...
        summary = BatchExecutionSummary(
            job_correlation_id="test-job",
            batch_id="batch-1",
            notebook_run_timestamp=base_time,
            total_queries=3,
            ...
            failed_uploads=2,
            total_records=150,
            total_uploaded_records=100,
            total_duration_seconds=10800,  # 3 hours
            time_range_start=base_time,
            time_range_end=base_time + timedelta(hours=3),
            executions=[execution1, execution2, execution3]
        )
        
        # Test generate_detailed_summary
        detailed = summary.generate_detailed_summary()
        
        # Verify overview section
        self.assertEqual(detailed['overview']['total_workspaces'], 2)
        self.assertEqual(detailed['overview']['total_unique_queries'], 2) 
        self.assertEqual(detailed['overview']['total_duration_seconds'], 10800)
        self.assertEqual(detailed['overview']['total_records_downloaded'], 150)
        self.assertEqual(detailed['overview']['total_records_uploaded'], 100)
        
        # Verify workspace query details
        self.assertEqual(len(detailed['workspace_query_details']), 3)  # 3 workspace/query combinations
        
        # Find details for workspace-1/query1
        ws1_q1 = next(d for d in detailed['workspace_query_details'] 
                     if d['workspaceId'] == 'workspace-1-guid' and d['query'] == 'query1')
        
        self.assertEqual(ws1_q1['logsDownloaded'], 100)
        self.assertEqual(ws1_q1['uploadSuccess'], 100)
        self.assertEqual(ws1_q1['uploadFailure'], 0)
        self.assertEqual(ws1_q1['avgQueryTime'], 5.5)
        self.assertEqual(ws1_q1['totalQueryTime'], 5.5)
        self.assertEqual(ws1_q1['queryExecutions'], 1)
        
        # Find details for workspace-2/query1  
        ws2_q1 = next(d for d in detailed['workspace_query_details']
                     if d['workspaceId'] == 'workspace-2-guid' and d['query'] == 'query1')
        
        self.assertEqual(ws2_q1['logsDownloaded'], 50)
        self.assertEqual(ws2_q1['uploadSuccess'], 0) 
        self.assertEqual(ws2_q1['uploadFailure'], 50)
        self.assertEqual(ws2_q1['avgQueryTime'], 3.0)
        
        # Find details for workspace-1/query2
        ws1_q2 = next(d for d in detailed['workspace_query_details']
                     if d['workspaceId'] == 'workspace-1-guid' and d['query'] == 'query2')
        
        self.assertEqual(ws1_q2['logsDownloaded'], 0)
        self.assertEqual(ws1_q2['uploadSuccess'], 0)
        self.assertEqual(ws1_q2['uploadFailure'], 0)
        self.assertEqual(ws1_q2['avgQueryTime'], 0.0)
        
        # Verify results are sorted by workspace then query
        details = detailed['workspace_query_details']
        for i in range(len(details) - 1):
            current = (details[i]['workspace'], details[i]['query'])
            next_item = (details[i + 1]['workspace'], details[i + 1]['query'])
            self.assertLessEqual(current, next_item)

    def test_kql_query_definition_set_stream(self):
        """Test KQLQueryDefinition.set_stream method - covers lines 89-90."""
        from sentinel_log_aggregator.models import KQLQueryDefinition
        ...
        query = KQLQueryDefinition(
            ...
            destination_stream="Custom-Test_CL",
            description="Test query",
            stream_name="Original-Stream_CL"
        )
        
        # Test set_stream method
        result = query.set_stream("New-Stream_CL")
        
        # Should return self for method chaining
        self.assertEqual(result, query)
        # Should update the stream_name
        self.assertEqual(query.stream_name, "New-Stream_CL")

    def test_kql_query_definition_build_query_none_parameters(self):
        """Test KQLQueryDefinition.build_query with None parameters - covers line 127."""
        from sentinel_log_aggregator.models import KQLQueryDefinition
        ...
        query.get_query = lambda: "SELECT * FROM table"
        
        # Test with None parameters - should initialize empty dict
        result = query.build_query(None)
        self.assertEqual(result, "SELECT * FROM table")

    def test_kql_query_definition_optional_parameter_no_default(self):
        """Test optional parameter with no default value - covers line 142."""
        from sentinel_log_aggregator.models import KQLQueryDefinition
        ...
        # Add optional parameter with no default
        query.add_parameter("optional_no_default", "string", required=False)
        query.get_query = lambda: "SELECT * WHERE field = '{optional_no_default}'"
        
        # Build query without providing the optional parameter
        result = query.build_query({})
        
        # Should use empty string as default
        self.assertIn("''", result)

    def test_workspace_config_properties(self):
        """Test WorkspaceConfig property methods - covers lines 42, 47-52, 57-62."""
        from sentinel_log_aggregator.models import WorkspaceConfig
        
        # Test with a complete resource ID
        config = WorkspaceConfig(
        ...
        self.assertEqual(config.subscription_id, "12345678-1234-1234-1234-123456789012")
        
        # Test resource_group property (lines 57-62)
        self.assertEqual(config.resource_group, "test-rg")
        
        # Test with invalid resource ID format to trigger ValueError exceptions
        ...
        self.assertEqual(config_invalid.subscription_id, "")
        
        # Should return empty string when 'resourcegroups' not found (line 61) 
        self.assertEqual(config_invalid.resource_group, "")

    def test_kql_query_definition_get_query(self):
        """Test KQLQueryDefinition.get_query method - covers line 94."""
        from sentinel_log_aggregator.models import KQLQueryDefinition
        
        # Create query with initial query string
        query = KQLQueryDefinition(
            name="test_query",
            destination_stream="Custom-Test_CL",
            description="Test query",
            stream_name="Custom-Test_CL",
            query="SELECT * FROM SecurityEvent"
        )
        
        # Test get_query method returns the internal query
        result = query.get_query()
        self.assertEqual(result, "SELECT * FROM SecurityEvent")

    def test_query_execution_properties(self):
        """Test QueryExecution property methods - covers lines 196, 206."""
        from sentinel_log_aggregator.models import QueryExecution, QueryStatus
        from datetime import datetime
        ...
        self.assertEqual(execution.status, QueryStatus.SUCCESS.value)

    def test_batch_execution_summary_properties(self):
        """Test BatchExecutionSummary property methods - covers lines 240-242, 247-250."""
        from sentinel_log_aggregator.models import BatchExecutionSummary
        from datetime import datetime
        
        # Test success_rate with zero queries (lines 240-242)
        summary_zero = BatchExecutionSummary(
            job_correlation_id="test-job",
            batch_id="batch-1", 
            notebook_run_timestamp=datetime.now(),
            total_queries=0,
            successful_queries=0,
            failed_queries=0,
            successful_uploads=0,
        ...
        # Should return 0.0 when total_queries is 0
        self.assertEqual(summary_zero.success_rate, 0.0)
        
        # Test upload_success_rate with zero uploads (lines 247-250)
        self.assertEqual(summary_zero.upload_success_rate, 0.0)
        
        # Test with actual values
        summary_normal = BatchExecutionSummary(
            job_correlation_id="test-job",
            batch_id="batch-1",
            notebook_run_timestamp=datetime.now(),
            total_queries=10,
        ...
        self.assertEqual(summary_normal.success_rate, 80.0)  # 8/10 * 100
        
        # Test upload_success_rate calculation 
        self.assertEqual(summary_normal.upload_success_rate, 87.5)  # 7/8 * 100
