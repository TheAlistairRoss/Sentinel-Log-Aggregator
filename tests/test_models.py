...
class TestQueryEnginePerformance:
    ...
    def mock_azure_client(self):
        ...
        client.upload_logs = AsyncMock(
            return_value=MagicMock(
                status=UploadStatus.SKIPPED, record_count=0, succeeded=True, error_message=None
            )
        )
        return client
...
class TestModelsMissingCoverage(unittest.TestCase):
    ...
    def test_kql_query_definition_to_dict_with_defaults(self):
        ...
        class TestQuery(KQLQueryDefinition):
            def __init__(self):
                ...
                self.add_parameter(
                    "param_with_default", "string", required=False, default="default_value"
                )
    ...
    def test_initialize_query_registry_exception_handling(self):
        ...
        with patch("sentinel_log_aggregator.query_registry.query_registry") as mock_registry:
            ...
            with patch(
                "sentinel_log_aggregator.models.AVAILABLE_QUERIES", {"test_query": mock_query_def}
            ):
    ...
    def test_kql_query_definition_build_query_edge_cases(self):
        ...
        query.get_query = (
            lambda: "SELECT * WHERE param = '{optional_param}' AND other = '{empty_optional}'"
        )
    ...
    def test_batch_execution_summary_get_detailed_summary(self):
        ...
        self.assertEqual(
            len(detailed["workspace_query_details"]), 3
        )  # 3 workspace/query combinations

        # Find details for workspace-1/query1
        ws1_q1 = next(
            d
            for d in detailed["workspace_query_details"]
