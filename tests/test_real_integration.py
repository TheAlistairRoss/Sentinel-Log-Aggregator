...
class TestRealAzureIntegration:
    ...
    def test_workspace_config(self):
        ...
        if not test_workspace_id or not test_workspace_customer_id:
            pytest.skip("TEST_WORKSPACE_ID and TEST_WORKSPACE_CUSTOMER_ID must be set in .env for integration tests")
        
        return {
            "resource_id": test_workspace_id,
            "customer_id": test_workspace_customer_id
        }
    ...
    async def test_workspace_connectivity(self, client_options, test_workspace_config):
        ...
        async with SentinelAggregatorClient(
            ...
            result = await client.query_workspace(
                workspace_id=test_workspace_config["customer_id"],
                query=test_query
            )
    ...
    async def test_query_without_params(self, client_options, test_workspace_config):
        ...
        workspace = WorkspaceConfig(
            ...
            parameters={
                "row_level_security_tag": "TEST_NO_PARAMS"
            }
        )
        ...
        async with SentinelAggregatorClient(
            ...
            assert summary.successful_queries > 0, f"No successful queries. Failed: {summary.failed_queries}"
    ...
    async def test_query_broken_syntax(self, client_options, test_workspace_config):
        ...
        workspace = WorkspaceConfig(
            ...
            parameters={
                "row_level_security_tag": "TEST_BROKEN_SYNTAX"
            }
        )
        ...
        async with SentinelAggregatorClient(
            ...
            if summary.total_queries > 0:
                ...
                assert summary.failed_queries == summary.total_queries, "Broken syntax query should fail"
    ...
    async def test_all_test_queries_together(self, client_options, test_workspace_config):
        ...
        async with SentinelAggregatorClient(
            ...
            assert summary.successful_queries >= 1, f"Expected at least 1 successful query, got {summary.successful_queries}"
            ...
            print(f"Records Downloaded: {detailed_summary['overview']['total_records_downloaded']:,}")
            ...
            for workspace_summary in detailed_summary.get('workspaces', []):
                ...
                for query_result in workspace_summary.get('queries', []):
                    print(f"    {query_result['query_name']}: {query_result['status']} - {query_result['records']} records")
    ...
    async def test_configuration_validation(self, client_options, test_workspace_config):
        ...
        print(f"  Batch settings: {client_options.days_ago} days, {client_options.batch_hours}h batches")