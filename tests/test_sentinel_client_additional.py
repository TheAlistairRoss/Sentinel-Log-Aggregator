...
from sentinel_log_aggregator.sentinel_client import BatchOperationPoller, SentinelAggregatorClient
...
class TestSentinelClientPropertyMocking:
    ...
    @pytest.mark.asyncio
    async def test_query_workspace_error_handling_via_private_field(
    ...
    @pytest.mark.asyncio
    async def test_query_workspace_http_error_via_private_field(
        self, mock_credential, client_options
    ):
        ...
        mock_query_client.query_workspace = AsyncMock(
            side_effect=HttpResponseError(
                "Workspace not found", response=MagicMock(status_code=404)
            )
        )
        client._logs_query_client = mock_query_client
...
class TestBatchOperationPollerCorrected:
    ...
    @pytest.mark.asyncio
    async def test_batch_operation_poller_result_property(
        self, mock_client, sample_workspaces, sample_query_def, sample_initial_result
    ):
    ...
    @pytest.mark.asyncio
    async def test_batch_operation_poller_flow(
        self, mock_client, sample_workspaces, sample_query_def, sample_initial_result
    ):
...
class TestSentinelClientMethodCoverage:
    ...
    async def test_validate_credentials_success(self, mock_credential, client_options):
        ...
        mock_credential.get_token = AsyncMock(return_value=AccessToken("valid-token", 1234567890))
