...
from sentinel_log_aggregator.models import WorkspaceConfig, KQLQueryDefinition, AVAILABLE_QUERIES
...
class TestSentinelClientSpecificLineCoverage:
    ...
    @pytest.mark.asyncio
    async def test_upload_logs_data_preparation_with_complex_data(self, mock_credential, client_options):
    ...
    async def test_query_workspace_with_various_exceptions(self, mock_credential, client_options):
        ...
        assert "authentication" in result.error_message.lower() or "failed" in result.error_message.lower()
    ...
    async def test_upload_logs_with_data_ingestion_error(self, mock_credential, client_options):
        ...
        # Verify error handling
        assert not result.succeeded
        assert "failed" in result.error_message.lower() or "ingestion" in result.error_message.lower()
