...
def sample_workspace_config():
    ...
    return WorkspaceConfig(
        ...
        queries_list=["tests_query_without_params", "tests_query_with_params"]
    )
...
class TestSentinelQueryEngine:
    ...
    def test_add_row_level_security_tag(self, query_engine):
        """Test adding row-level security tags to data."""
        data = [
            {"TimeGenerated": "2023-01-01T00:00:00Z", "IncidentNumber": 1001},
            {"TimeGenerated": "2023-01-01T00:01:00Z", "IncidentNumber": 1002}
        ]
        ...
        transformed_data = query_engine.add_row_level_security_tag(data, workspace_alias)
        
        assert len(transformed_data) == 2
        assert all("RowLevelSecurityTag" in record for record in transformed_data)
        assert all(record["RowLevelSecurityTag"] == workspace_alias for record in transformed_data)
        
        # Original data should be preserved
        assert transformed_data[0]["IncidentNumber"] == 1001
        assert transformed_data[1]["IncidentNumber"] == 1002
    
    def test_add_row_level_security_tag_empty_data(self, query_engine):
        """Test adding row-level security tags to empty data."""
        data = []
        workspace_alias = "test-workspace"
...
class TestQueryEnginePerformance:
    ...
    def test_memory_usage_large_datasets(self, query_engine):
        ...
        # Transform the data
        transformed = query_engine.add_row_level_security_tag(large_dataset, "test")
        
        # Verify transformation completed
        assert len(transformed) == 10000
        assert all("RowLevelSecurityTag" in record for record in transformed[:10])
...
class TestSentinelAggregatorClient:
    ...
    async def test_query_workspace_success(self, client):
        ...
        with patch.object(client, '_logs_query_client_instance') as mock_logs_client:
    ...
    async def test_upload_logs_success(self, client):
        ...
        with patch.object(client, '_logs_ingestion_client_instance') as mock_ingestion_client:
...
class TestWorkspaceConfigValidation:
    ...
    def test_invalid_report_name(self):
        ...
        assert 'invalid report name' in str(exc_info.value).lower()
...
class TestWorkspaceSet:
    ...
    def sample_workspaces(self):
        ...
        return [
            WorkspaceConfig(
                ...
                customer_id="11111111-1111-1111-1111-111111111111",
                parameters={"row_level_security_tag": "prod"},
                queries_list=["report_incident_summary", "report_workspace_usage"]
            ),
            WorkspaceConfig(
                resource_id="/subscriptions/12345678-1234-1234-1234-123456789abc/resourcegroups/dev-rg/providers/microsoft.operationalinsights/workspaces/dev-workspace",
                customer_id="22222222-2222-2222-2222-222222222222",
            ...
            WorkspaceConfig(
                ...
                parameters={"row_level_security_tag": "test"},
                queries_list=["report_workspace_usage"]
            )
        ]
    ...
    def test_first_id(self, workspace_set):
        """Test getting first workspace ID."""
        first_id = workspace_set.first_id()
        assert first_id == "11111111-1111-1111-1111-111111111111"
    
    def test_first_alias(self, workspace_set):
        """Test getting first workspace alias."""
        first_alias = workspace_set.first_alias()
        assert first_alias == "prod"
    
    def test_additional_ids(self, workspace_set):
        """Test getting additional workspace IDs."""
        additional = workspace_set.additional_ids()
        expected = [
            "22222222-2222-2222-2222-222222222222",
            "33333333-3333-3333-3333-333333333333"
        ]
    ...
    def test_filter_by_alias(self, workspace_set):
        ...
        assert filtered.count() == 1
        assert filtered.first_id() == "22222222-2222-2222-2222-222222222222"
    
    def test_has_report(self, workspace_set):
        """Test filtering by report."""
        incident_workspaces = workspace_set.has_report("report_incident_summary")
        assert incident_workspaces.count() == 2
        assert incident_workspaces.aliases() == ["prod", "dev"]
        
        usage_workspaces = workspace_set.has_report("report_workspace_usage")
    ...
    def test_details(self, workspace_set):
        ...
        assert set(first_detail['reports']) == {"report_incident_summary", "report_workspace_usage"}
    ...
    def test_empty_workspace_set(self):
        ...
        assert empty_set.first_id() is None
        assert empty_set.first_alias() is None
...
class TestWorkspaceManager:
    ...
    def test_for_report(self, sample_workspaces):
        ...
        incident_workspaces = manager.for_report("report_incident_summary")
        assert incident_workspaces.count() == 1
        assert incident_workspaces.aliases() == ["prod"]
        
        usage_workspaces = manager.for_report("report_workspace_usage")
    ...
    def test_unique_reports(self, sample_workspaces):
        ...
        expected_reports = {"report_incident_summary", "report_workspace_usage"}
