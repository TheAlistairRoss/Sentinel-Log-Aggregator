...
class TestEnvironmentVariableLoading:
    ...
    def test_load_environment_variables_default_file_exists(self):
        ...
        with patch('sentinel_log_aggregator.cli.Path.exists', return_value=True), \
    ...
    def test_load_environment_variables_default_file_not_exists(self):
        ...
        with patch('sentinel_log_aggregator.cli.Path.exists', return_value=False), \
             patch('sentinel_log_aggregator.cli.load_dotenv') as mock_load_dotenv:
...
class TestArgumentParser:
    ...
    def test_parser_global_arguments(self):
        ...
        args = parser.parse_args([
            '--log-level', 'DEBUG',
            '--dcr-endpoint', 'https://test.ingest.monitor.azure.com',
            '--dcr-rule-id', 'dcr-test',
            'health',
            '--workspace-config', 'test.yaml'
        ])
    ...
    def test_parser_invalid_log_level(self):
        ...
        with pytest.raises(SystemExit):
            parser.parse_args(['--log-level', 'INVALID', 'health', '--workspace-config', 'test.yaml'])
...
class TestMainFunction:
    ...
    @pytest.mark.asyncio
    async def test_main_health_command_success(self, mock_health, mock_load_ws, mock_create_options, 
                                             mock_setup_logging, mock_load_env, mock_workspaces, mock_client_options):
    ...
    @patch('sys.argv', ['sentinel-aggregator', 'run', '--workspace-config', 'test.yaml', '--days-back', '7'])
    @pytest.mark.asyncio
    async def test_main_run_command_success(self, mock_run, mock_load_ws, mock_create_options,
                                          mock_setup_logging, mock_load_env, mock_workspaces, mock_client_options):
        ...
        assert result == 0
        mock_run.assert_called_once_with(mock_client_options, mock_workspaces, 7, mock_client_options.batch_hours)
    ...
    @pytest.mark.asyncio
    async def test_main_validate_command_success(self, mock_load_ws, mock_create_options,
                                               mock_setup_logging, mock_load_env, mock_workspaces, mock_client_options):
    ...
    @patch('sys.argv', ['sentinel-aggregator', 'health', '--workspace-config', 'test.yaml'])
    @pytest.mark.asyncio
    async def test_main_unexpected_error(self, mock_create_options, mock_setup_logging, mock_load_env):
...
class TestConfigFileLoading:
    ...
    @patch('sentinel_log_aggregator.cli.load_workspace_config')
    @patch('sys.argv', ['sentinel-aggregator', '--config-file', 'config.yaml', 'validate', '--workspace-config', 'test.yaml'])
    @pytest.mark.asyncio
    async def test_main_with_config_file(self, mock_load_ws, mock_from_yaml, mock_setup_logging, mock_load_env, mock_client_options):
...
class TestRunAggregation:
    ...
    async def test_run_aggregation_success(self):
        ...
        with patch('sentinel_log_aggregator.client_options.SentinelAggregatorClientOptions.validate', return_value=[]):
            with patch('sentinel_log_aggregator.cli.SentinelAggregatorClient') as mock_client_class:
                mock_client = AsyncMock()
                mock_client_class.return_value.__aenter__.return_value = mock_client
                ...
                with patch('sentinel_log_aggregator.cli.SentinelQueryEngine') as mock_engine_class:
                    ...
                    mock_engine.execute_batch_queries_with_streaming_upload.return_value = mock_summary
    ...
    async def test_run_aggregation_config_validation_failure(self):
        ...
        with patch('sentinel_log_aggregator.client_options.SentinelAggregatorClientOptions.validate', return_value=["Missing required field"]):
        ...
        with patch('sentinel_log_aggregator.client_options.SentinelAggregatorClientOptions.validate', return_value=[]):
            with patch('sentinel_log_aggregator.cli.SentinelAggregatorClient') as mock_client_class:
                ...
                with patch('sentinel_log_aggregator.cli.SentinelQueryEngine') as mock_engine_class:
                    mock_engine = AsyncMock()
                    mock_engine_class.return_value = mock_engine
                    mock_engine.execute_batch_queries_with_streaming_upload.side_effect = Exception("Execution failed")
...
class TestMainCommandHandling:
    ...
    async def test_main_health_command(self):
        ...
        with patch('sentinel_log_aggregator.cli.create_parser') as mock_create_parser:
            ...
            with patch('sentinel_log_aggregator.cli.check_service_health', new_callable=AsyncMock) as mock_health:
                with patch('sentinel_log_aggregator.cli.create_client_options_from_args') as mock_create_opts:
                    mock_opts = MagicMock()
    ...
    async def test_main_run_command(self):
        ...
        with patch('sentinel_log_aggregator.cli.create_parser') as mock_create_parser:
            ...
            with patch('sentinel_log_aggregator.cli.run_aggregation', new_callable=AsyncMock) as mock_run:
                with patch('sentinel_log_aggregator.cli.create_client_options_from_args') as mock_create_opts:
        ...
        with patch('sentinel_log_aggregator.cli.create_parser') as mock_create_parser:
            ...
            with patch('sentinel_log_aggregator.cli.create_client_options_from_args') as mock_create_opts:
                with patch('sentinel_log_aggregator.cli.load_workspace_config') as mock_load_ws:
                    ...
                    mock_workspaces = [
                        MagicMock(customer_id="11111111-1111-1111-1111-111111111111", workspace_name="test-ws")
                    ]
                    mock_load_ws.return_value = mock_workspaces
        ...
        with patch('sentinel_log_aggregator.cli.create_parser') as mock_create_parser:
            ...
            with patch('sentinel_log_aggregator.cli.create_client_options_from_args') as mock_create_opts:
                with patch('sentinel_log_aggregator.cli.load_workspace_config', return_value=[]):
                    ...
                    mock_opts = MagicMock()
                    mock_opts.validate.return_value = ["Missing required field"]  # Validation errors
                    mock_create_opts.return_value = mock_opts
    ...
    async def test_main_unexpected_exception(self):
        ...
        with patch('sentinel_log_aggregator.cli.create_parser') as mock_create_parser:
            ...
            with patch('sentinel_log_aggregator.cli.create_client_options_from_args') as mock_create:
