...
class TestModelsComplete:
    ...
    def test_workspace_config_properties_lines_42_62(self):
        ...
        config4 = WorkspaceConfig(
            resource_id="/subscriptions/12345/invalid", customer_id="customer-123"
        )
    ...
    def test_from_yaml_method_lines_213_216(self):
        ...
        yaml_content = {
            ...
            "parameters": {
                "event_id": {"type": "int", "required": True, "description": "Event ID to filter"},
                "limit": {
                    "type": "int",
                    "required": False,
                    "default": "50",
    ...
    def test_query_execution_summary_get_detailed_summary_lines_259_344(self):
        ...
        assert (
            len(detailed_summary["workspace_query_details"]) == 2
        )  # Two workspace-query combinations        # Verify overview section
        ...
        assert (
            len(details) == 2
        )  # Two unique workspace-query combinations (workspace1 has 1 failed + 1 success for same query = 1 group, workspace2 has 1 success for different query = 1 group)
    ...
    def test_comprehensive_model_edge_cases(self):
        ...
        param_required = QueryParameter(
            param_type="int", required=True, description="Required parameter"
        )
