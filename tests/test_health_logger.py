...
from sentinel_log_aggregator.models import QueryExecution, QueryStatus, UploadStatus, WorkspaceConfig
...
class TestSentinelAggregatorHealthLogger:
    ...
    def test_initialization_disabled(self, mock_sentinel_client):
        ...
        logger = SentinelAggregatorHealthLogger(
            sentinel_client=mock_sentinel_client,
            enabled=False
        )
    ...
    @pytest.mark.asyncio
    async def test_log_query_execution(self, health_logger, mock_sentinel_client, test_query_execution, test_workspace_config):
    ...
    @pytest.mark.asyncio
    async def test_log_query_execution_failed(self, health_logger, mock_sentinel_client, test_workspace_config):
        ...
        await health_logger.log_query_execution(
            job_id=job_id,
            query_execution=failed_execution,
            workspace_config=test_workspace_config
        )
    ...
    @pytest.mark.asyncio
    async def test_log_workspace_processing_start(self, health_logger, mock_sentinel_client, test_workspace_config):
    ...
    @pytest.mark.asyncio
    async def test_log_workspace_processing_end(self, health_logger, mock_sentinel_client, test_workspace_config):
    ...
    @pytest.mark.asyncio
    async def test_extended_properties_json_serialization(self, health_logger, mock_sentinel_client):
        ...
        extended_props = json.loads(record["ExtendedProperties"])
        assert extended_props["timestamp"] == "2024-01-01 12:00:00+00:00"  # str() format, not isoformat()
