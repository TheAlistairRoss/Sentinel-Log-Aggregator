...
class TestQueryEngineMissingCoverage:
    ...
    def test_build_query_with_parameters_invalid_query(self):
        ...
        with pytest.raises(ValueError, match="Query 'invalid_query' not found in available queries"):
    ...
    def test_build_query_with_parameters_valid_query(self):
        ...
        result = engine.build_query_with_parameters("query_incident_summary", {"parameters": {"row_level_security_tag": "TEST"}})
    ...
    def test_build_query_from_name_invalid_query(self):
        ...
        # Test with invalid query name
        with pytest.raises(KeyError, match="Query 'nonexistent_query' not found in available queries"):
    ...
    async def test_execute_single_query_exception_handling(self):
        ...
        from sentinel_log_aggregator.query_engine import SentinelQueryEngine
        from sentinel_log_aggregator.client_options import SentinelAggregatorClientOptions
        from sentinel_log_aggregator.models import WorkspaceConfig, QueryStatus, UploadStatus
    ...
    async def test_execute_single_query_memory_cleanup(self):
        ...
        mock_client = AsyncMock()
        mock_client.query_workspace = AsyncMock(return_value=MagicMock(
            status=QueryStatus.SUCCESS,
            data=[{"TimeGenerated": "2023-01-01T00:00:00Z", "IncidentNumber": 1001}],
            record_count=1,
            succeeded=True,
        ...
        mock_client.upload_logs = AsyncMock(return_value=MagicMock(
            status=UploadStatus.SUCCESS,
            succeeded=True,
            record_count=1
        ))
    ...
    async def test_execute_batch_queries_workspace_processing(self):
        ...
        detailed_summary_mock = {
            "overview": {"total_queries": 3},
            "workspace_query_details": []
        }
        with patch('sentinel_log_aggregator.models.BatchExecutionSummary.generate_detailed_summary', return_value=detailed_summary_mock), \
    ...
    async def test_execute_batch_queries_large_dataset_cleanup(self):
        ...
        large_data = [{"TimeGenerated": f"2023-01-01T00:0{i%60:02d}:00Z", "Count": i} for i in range(1000)]
        mock_client = AsyncMock()
        mock_client.query_workspace = AsyncMock(return_value=MagicMock(
            status=QueryStatus.SUCCESS,
            data=large_data,
...
class TestQueryEngineCompleteCoverage:
    ...
    async def test_batch_execution_query_build_exception(self):
        ...
        assert "Query build error: Parameter validation failed" in failed_execution.query_error_message
    ...
    async def test_batch_execution_critical_error_detection(self):
        ...
        workspace = WorkspaceConfig(
            ...
            customer_id="test-workspace-id",
            parameters={"row_level_security_tag": "TEST"},
            queries_list=["query_incident_summary", "query_workspace_usage"]  # Multiple queries to test batching
        )

        detailed_summary_mock = {
            "overview": {"total_queries": 2},
            "workspace_query_details": []
        }
        ...
        mock_logger.error.assert_any_call("SYNTAX", "1 syntax error(s) detected - stopping execution")
    ...
    async def test_critical_error_final_logging(self):
        ...
        mock_logger.error.assert_any_call("CRITICAL_STOP", "EXECUTION STOPPED DUE TO CRITICAL ERRORS")
        mock_logger.error.assert_any_call("ACTION_REQUIRED", "Fix syntax errors in KQL queries before retrying")
...
class TestQueryEngineRemainingLines:
    ...
    async def test_batch_execution_critical_early_break_lines_388_389(self):
        ...
        from sentinel_log_aggregator.query_engine import SentinelQueryEngine
        from sentinel_log_aggregator.client_options import SentinelAggregatorClientOptions
        from sentinel_log_aggregator.models import WorkspaceConfig, QueryExecution, QueryStatus, UploadStatus
        ...
        mock_client.upload_logs = AsyncMock(return_value=MagicMock(succeeded=True, record_count=1, upload_time=0.1))
        ...
        critical_logged = any("Critical errors detected - stopping execution" in call or 
                             "CRITICAL_STOP" in call or 
                             "EXECUTION STOPPED DUE TO CRITICAL ERRORS" in call 
    ...
    async def test_final_coverage_lines_245_388_389(self):
        ...
        from sentinel_log_aggregator.query_engine import SentinelQueryEngine
        from sentinel_log_aggregator.client_options import SentinelAggregatorClientOptions
        from sentinel_log_aggregator.models import WorkspaceConfig, QueryExecution, KQLQueryDefinition, QueryStatus, UploadStatus
        import gc
        ...
        mock_successful_query.data = [{"TimeGenerated": "2024-01-01T00:00:00Z", "Computer": "TEST", "test": "data"}]
        ...
        with patch.object(engine, 'logger') as mock_logger:
            ...
            syntax_errors = [
                ...
                if e.query_status == QueryStatus.FAILED.value and any(
                    error_type in e.query_error_message 
                    for error_type in ['BadArgumentError', 'QueryCompilationError', 'SyntaxError', 'SemanticError']
                )
            ]
            
            # This should trigger the SYNTAX error logging
            if syntax_errors:
                engine.logger.error("SYNTAX", f"{len(syntax_errors)} syntax error(s) detected - stopping execution")
                for error_exec in syntax_errors:
                    engine.logger.error("SYNTAX_DETAIL", error_exec.query_error_message, query_name=error_exec.query_name)
