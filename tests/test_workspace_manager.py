...
class TestQueryEnginePerformance:
    ...
    def test_memory_usage_large_datasets(self, query_engine):
        ...
        # Test that the engine has proper memory management capabilities
        # This would involve monitoring memory usage during processing
        
        # Verify engine has memory management features
        assert hasattr(query_engine, 'client_options')
        assert hasattr(query_engine.client_options, 'max_concurrent_queries')
        
        # Simulate large dataset processing
        large_dataset_size = 10000
        batch_size = query_engine.client_options.max_concurrent_queries
        
        # Verify batching logic exists for memory efficiency
        assert batch_size > 0
        expected_batches = large_dataset_size // batch_size
        assert expected_batches > 0
        
        # In a real test, we would monitor actual memory usage here
...
class TestWorkspaceManager:
    ...
    def test_unique_queries(self, sample_workspaces):
        ...
        manager = WorkspaceManager(sample_workspaces)
        queries = manager.unique_reports()
        expected_queries = {"query_incident_summary", "query_workspace_usage"}
